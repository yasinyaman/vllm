...
class MoERunner(MoERunnerInterface):
    ...
    def _maybe_stabilize_output(
        ...
        See the contract note in ``__init__``. Sharing one buffer per (role,
        width, dtype) across layers is safe: every consumer of layer i's
        output is enqueued by piece i+1's replay before layer i+1's moe --
        the next writer -- runs, and a single stream orders the two.
        """
